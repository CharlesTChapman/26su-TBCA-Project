...
LABEL_MAP = {
    ...
    'predicted':            'Predicted Employment (avg)',
}

tab1, tab2, tab3, tab4 = st.tabs(
    ['Employment Trend', 'Country Comparison', 'Absorption Rate', 'Custom Scatter'])
...
with tab3:
    ...
    fig3 = px.bar(absorption, x='absorption_rate', y='sector', orientation='h',
                  ...
                  labels={'absorption_rate': 'Median New Jobs per Prior-Year Graduate',
                          'sector': 'Sector'})
    fig3.add_vline(x=0, line_color='black', line_width=1)
    st.plotly_chart(fig3, use_container_width=True)
    st.caption('Positive = sector absorbs more workers than graduates produced. '
               'Negative = graduate surplus.')
...
with tab4:
    ...
    mask = df[t4_x].notna() & df[t4_y].notna()
    x = df.loc[mask, t4_x].to_numpy(dtype=float)
    y = df.loc[mask, t4_y].to_numpy(dtype=float)
    if len(x) >= 2 and t4_x != t4_y:
        ...
        fig4.add_scatter(x=x_line, y=slope * x_line + intercept,
                         mode='lines', name='OLS fit',
                         line=dict(color='black', width=2),
                         hovertemplate=(f'y = {slope:.2f}x + {intercept:.2f}<br>'
